...
export default function App() {
    ...
    const defaultPrompt = `
    Generate a resume for John Doe. 
    Contact: john.doe@example.com, 555-123-4567, linkedin.com/in/johndoe, github.com/johndoe.
    ...
    const handleGenerateResume = async () => {
        ...
        try {
            const apiKey = "AIzaSyCe_rF1STbrmnrVpqsm7y9S9XJJPE2Wm3A"; // API key is handled by the environment
    ...
    const handleImproveSection = async (type, data, index = null) => {
        ...
        try {
            const apiKey = "AIzaSyCe_rF1STbrmnrVpqsm7y9S9XJJPE2Wm3A"; // Handled by environment
            const apiUrl = `https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash:generateContent?key=${apiKey}`;
            const response = await fetch(apiUrl, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) });
    ...
    const handleDownloadPdf = () => {
        // Correctly access the jsPDF constructor from the global window object
        const jsPDF = window.jsPDF;
        const html2canvas = window.html2canvas;

        if (!jsPDF || !html2canvas) { 
            setError("PDF generation libraries are not available. Please try again later.");
